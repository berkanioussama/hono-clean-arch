...
export default class FirstName {
    ...
    public static create(firstName: string): FirstName {
        ...
        if (trimmed.length < FirstName.MIN_LENGTH) {
            throw new ValidationError(`First name must be at least ${FirstName.MIN_LENGTH} characters`);
        }
        if (trimmed.length > FirstName.MAX_LENGTH) {
            throw new ValidationError(`First name cannot exceed ${FirstName.MAX_LENGTH} characters`);
        }
        if (!FirstName.NAME_REGEX.test(trimmed)) {
            throw new ValidationError("First name can only contain letters, spaces, hyphens, and apostrophes");
        }
